...
// ==========================================
// 1. 3D HOLOGRAPHIC TILT COMPONENT (Brought back for the "Cool" factor)
// ==========================================
...
export default function SaralShaktiPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-300 font-sans selection:bg-emerald-500/30 relative overflow-hidden">
      ...
      {/* --- THE AURORA BACKGROUND (Dark Mode + FMCG Tints) --- */}
          ...
          <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <div className="relative group cursor-pointer">
                ...
                <button className="relative w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3.5 bg-black rounded-full text-white border border-white/10 font-medium">
                  <Calculator className="w-4 h-4 text-emerald-400" />
                  Calculate Family Need
                </button>
              </div>
              <button className="flex items-center justify-center gap-2 px-8 py-3.5 rounded-full bg-white/5 hover:bg-white/10 text-zinc-300 font-medium border border-white/10 transition-all">
                Explore the Logic
              </button>
            </div>
          </div>
          
          {/* Hero Visual Mockup (Using TiltCard for "Cool" factor) */}
          <div className="relative lg:pl-12">
            <TiltCard>
              <div className="relative bg-zinc-950/80 p-8 rounded-3xl border border-white/10 shadow-2xl backdrop-blur-md overflow-hidden group">
                {/* Techy Scanline overlay on card */}
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <div className="relative group cursor-pointer">
          {/* Neon Glow */}
          <div className="absolute -inset-4 bg-emerald-500/20 rounded-full blur-xl group-hover:bg-emerald-500/40 transition-all duration-500 animate-pulse" />
          
          {/* Button */}
          <div className="w-14 h-14 sm:w-16 sm:h-16 bg-zinc-950 rounded-full shadow-[0_0_20px_rgba(0,0,0,0.5)] flex items-center justify-center border border-white/10 transform transition-transform group-hover:scale-110 active:scale-95 relative z-10">
             <div className="absolute inset-0.5 rounded-full bg-gradient-to-r from-emerald-500/20 to-amber-500/20 opacity-50" />
             <Bot className="w-6 h-6 text-emerald-400 relative z-10" />
             <span className="absolute top-0 right-0 h-3 w-3 sm:h-4 sm:w-4 bg-emerald-500 rounded-full border-2 border-black animate-bounce" />
          </div>
          
          {/* Tooltip */}
          <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 w-max px-4 py-2 bg-zinc-900/90 backdrop-blur-md rounded-xl border border-white/10 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none shadow-xl">
            <p className="text-xs font-medium text-white flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"/> Ask the Saral Nutritionist
            </p>
          </div>
        </div>
      </div>
