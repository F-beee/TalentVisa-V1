...
const TiltCard = ({ children, className }: { children: React.ReactNode, className?: string }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    // Disable heavy 3D effects on mobile devices for better UX/Performance
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-300 font-sans selection:bg-blue-500/30 relative overflow-hidden pb-24">
          ...
          <div className="lg:col-span-4 flex flex-col gap-3 relative z-10">
            {Object.entries(treeData).map(([key, data]) => (
              <button
                ...
                className={`w-full flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl transition-all duration-300 text-left border relative overflow-hidden ${activeNode === key
      ...
      <AiAssistant
        talentData={{
          name: "Visitor",
          skills: { coding: 99, analytics: 100 }
        }}
      />
    </div>
  )
